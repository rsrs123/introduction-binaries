...
case "$OS" in
  darwin)
    ...
    VSIX_URL="https://github.com/$REPO/releases/download/$TAG/int-ai-extension-0.2.3.vsix"
    ...
    if curl -fsSL -o "$TMP/int-ext.vsix" "$VSIX_URL" 2>/dev/null && [ -x "$INTRO_CLI" ]; then
      say "$CYAN" "→ Instalando int-ai-extension v0.2.3..."
      if "$INTRO_CLI" --install-extension "$TMP/int-ext.vsix" --force; then
        ok "int-ai-extension v0.2.3 instalada (wizard refinado + Qdrant suppress persistente)"
      else
